...
#             its feature-named test/e2e/<feature>_test.go file (carrying
#             the '(D-<slug>)' header comment that maps it to the doc page).
...
def plan_prompt() -> str:
    ...
    return (
        ...
        "sources and the doc paths they cover. The doc-source e2e tests are "
        "feature-named (for example test/e2e/quickstart_flow_test.go), never "
        "test/e2e/docs_<slug>_test.go, which the committed gate at "
        "test/e2e/naming_convention_test.go forbids. Each feature-named file "
        "carries a header comment of the form "
        "'// End-to-end tests for <doc path> (D-<slug>).' (the doc path may "
        "wrap to the next comment line, with the '(D-<slug>)' on it, and a "
        "glob path covers a directory of pages). Build the catalogue of "
        "D-<slug> sources from TEST-GAPS.md, then treat a source as already "
        "built when grep finds its '(D-<slug>)' tag in any "
        + REPO + "/test/e2e/*_test.go header, e.g. "
        "`grep -rl '(D-<slug>)' " + REPO + "/test/e2e/`. Group the sources "
        "that are NOT yet built (no feature-named file carries their "
        "'(D-<slug>)' tag) into a few doc batches of related sources (for "
        "example doc-getting-started, doc-authoring, doc-consuming, "
        "doc-deployment), each listing its docSources.\n\n"
...
def doc_prompt(batch: dict) -> str:
    ...
    return (
        ...
        "test/e2e/, one file per source. Name each file for the feature or "
        "behavior under test (for example test/e2e/quickstart_flow_test.go), "
        "NOT test/e2e/docs_<slug>_test.go -- the committed gate at "
        "test/e2e/naming_convention_test.go forbids a docs_ filename and a "
        "numeric Test<Word>_<digit>... function name. Open each file with the "
        "convention header comment "
        "'// End-to-end tests for <doc path> (D-<slug>).' (use the glob path "
        "form when one file covers a directory of pages), and put a "
        "'// T-D-<slug>-N -- ...' comment above each test function. Drive the "
        "real surface -- build and exec the CLI (cmd/podium), spawn the MCP "
        "server over stdio (cmd/podium-mcp), or start the HTTP server "
        "(cmd/podium-server) -- and assert the observable outcomes the "
        "documentation and spec claim. Obey the ANTI-HANG rule: the test owns "
        "every process lifecycle with timeouts and teardown.\n"

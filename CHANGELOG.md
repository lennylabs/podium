...
- The registry reports the configuration at startup with the line
  `SCIM group expansion in layer visibility: <bool> (identity provider "<name>")`.
- `GET /v1/admin/show-effective` distinguishes a layer admitted through the
  SCIM-pushed directory from one admitted through the caller's own group claim.

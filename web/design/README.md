...
The only surface with writes. Its contents depend on what the caller may read under §7.3.1, and its per-row rendering differs by layer class rather than by caller role, because no response reports the caller's role.

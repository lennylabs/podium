...
### Plugin grouping and the plugin descriptor

A plugin that bundles several artifacts cannot be expressed by the current per-artifact adapter call, so this proposal adds the target plugin to the rendering input. The `Source` struct (`pkg/adapter/adapter.go`) gains a plugin descriptor: the plugin name, an optional description, and the harness subtree prefix. A marketplace emitter reads it to write under `<harness>/<plugin>/...` and to contribute the correct manifest fragment, replacing the artifact-keyed `plugins/<artifact-id>/` path the cowork adapter uses today. The descriptor is set only for marketplace publishing; the project-files mode (`podium sync` and `load_artifact`) leaves it at its zero value, which the project-files adapters ignore, so their output is unchanged. The publishing pipeline assigns each selected artifact to its plugin by evaluating the plugin scope filters in declaration order, then invokes the emitter per artifact with the resolved plugin descriptor for the artifact's component files. The per-plugin manifest entry is contributed once per plugin keyed by the plugin name rather than once per artifact, so an N-artifact plugin yields one plugin entry rather than N duplicates under the array-concatenating merge (Marketplace emitters below). Marketplace publishing introduces no new SPI: the marketplace emitter is an internal, unregistered interface that reuses the §6.7 layout helpers, and it is absent from the §9.1 SPI table. For the tap and package formats (Hermes, Pi), where the install unit is the individual skill, the plugin descriptor groups skills into subtrees but does not change the install unit.

### Marketplace emitters per harness

A marketplace emitter is a separate internal interface (`MarketplaceEmitter`, `pkg/adapter/emitter.go`) from the `HarnessAdapter` SPI. It splits rendering into two methods so the per-plugin manifest entry is contributed once per plugin rather than once per artifact: `Component(ctx, Source)` renders one artifact's component files under the plugin subtree, called once per artifact, and `Manifest(ctx, marketplaceName, PluginDescriptor)` renders the root marketplace entry and the per-plugin manifest, called once per plugin. The publishing pipeline selects an emitter per harness rather than the project-files adapter:

- **Claude (Code, Desktop, Cowork).** One emitter, reusing the §6.7 layout helpers, with the plugin root the resolved plugin name under the harness subtree and the marketplace name the output's operator-chosen identifier. The marketplace fragment carries one entry per plugin keyed by plugin name, contributed once per plugin rather than once per artifact, because several artifacts now map to one plugin. The `PodiumOwnedKey` tag is keyed on the plugin name, and `Manifest` renders the plugin entry from the plugin descriptor (§"Plugin grouping and the plugin descriptor") rather than from each artifact, so the one `Manifest` call per plugin produces one plugin entry. The split is required because the `OpMergeJSON` merge concatenates same-key arrays without deduplication within a render (`deepMerge`, `pkg/materialize/merge.go:238`), so a per-artifact fragment would emit N duplicate entries for an N-artifact plugin. The three Claude surfaces consume the one emitted `.claude-plugin/marketplace.json`.
...
4. Document the plugin descriptor: the `Source` struct carries a plugin descriptor (name, optional description, harness subtree prefix) so a marketplace emitter can render an artifact into a named plugin. The `HarnessAdapter.Adapt` method signature is unchanged, and publishing introduces no new SPI (the marketplace emitter is an internal, unregistered interface).

...
A user-defined layer's owner and visibility are fixed at the registration that created it. On `podium layer update` against such a layer, the refusal reads the value each flag would store against the value the layer holds. A flag that would change the stored record is refused with `registry.invalid_argument` carrying `details.constraint: "immutable_visibility"`: `--public`, `--organization`, `--group`, a `--user` naming anyone other than the owner, an `--owner` naming another subject, and `--clear-users`, which empties the stored `users: [<owner>]`. The refusal rejects the whole patch, so no other flag the same command carries is applied. A flag whose value restates what the layer holds asserts nothing and is admitted, including `--public=false`, `--organization=false`, `--user <owner>`, `--owner <owner>`, and `--clear-groups` against a layer storing no group. The rule reads the stored layer's class rather than the caller, so it refuses whoever runs the command: the layer's owner, a tenant admin, and every caller on a deployment with no identity provider configured or one in public mode. An administrator who needs the layer visible more widely re-registers its ID with `podium layer register` as an admin-defined layer carrying the visibility flags they want. That re-registration replaces the stored record: the layer's registration time is the time of the re-registration, its order is recomputed at the tail of the layer list, its `last_ingested_ref` and `last_ingested_at` are emptied so the next ingest reads the source afresh, a `git` source is issued a new inbound webhook secret that must be registered at the Git host in place of the old one, and the former owner regains a slot against the per-identity user-defined layer cap. An operator on a deployment with no identity provider who needs to correct an `--owner` value has the same recourse: re-register the ID with `podium layer register --user-defined --owner <corrected-sub>`, which that deployment admits on the admin arm.

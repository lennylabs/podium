...
For a quick stand-up, the repo ships a `docker-compose.yml` that brings up the evaluation stack with `docker compose up -d`: the registry, a pgvector Postgres, MinIO object storage, a Dex OIDC IdP, and a one-shot bootstrap container that creates the MinIO bucket. The registry service inherits the Postgres-backed embedding default of `openai`, so add `OPENAI_API_KEY` to its `environment:` block before starting the stack, or add `PODIUM_EMBEDDING_PROVIDER: "none"` there to run BM25-only search; without one of the two the registry exits with `missing required configuration for the selected backend(s): OPENAI_API_KEY`. The registry seeds the first tenant and admin grant itself at boot, from the default tenant plus the identity in `PODIUM_BOOTSTRAP_ADMINS`. The stack runs single-replica services with default credentials on local volumes, so it is unsuitable for production. It wires the same components a clustered deployment wires, so consumers exercise the same code paths.
...
Three chart defaults need attention before the install works. `values.yaml` sets `config.identityProvider.type` to `oauth-device-code`, which the registry refuses at startup with `config.identity_provider_unverified`, so pass `--set config.identityProvider.type=oidc-jwt`. The container `env:` block takes precedence over `envFrom:`, so `PODIUM_IDENTITY_PROVIDER` has to come from `--set` rather than from the secret. `values.yaml` also sets `config.embeddingProvider.type` to `openai`, and a registry that selects `openai` without `OPENAI_API_KEY` exits with `missing required configuration for the selected backend(s): OPENAI_API_KEY`. Supply the key in the secret, or pass `--set config.embeddingProvider.type=none` to run BM25-only search. The third is `config.bind`: `values.yaml` sets it to `0.0.0.0:8080`, but no template renders that value, so the registry falls back to its `127.0.0.1:8080` default and the kubelet's `/healthz` and `/readyz` probes cannot reach the pod. `--set config.bind=...` has no effect for the same reason, so set `PODIUM_BIND=0.0.0.0:8080` in the secret as the command above does.

The templates set `PODIUM_REGISTRY_STORE`, `PODIUM_OBJECT_STORE`, `PODIUM_VECTOR_BACKEND`, `PODIUM_EMBEDDING_PROVIDER`, and `PODIUM_IDENTITY_PROVIDER` from those `type` values, and the secret supplies the DSN, bucket, region, API keys, and OIDC settings through `envFrom`. The remaining keys in `values.yaml`, including `config.store.dsn` and `config.identityProvider.audience`, are placeholders that no template renders.

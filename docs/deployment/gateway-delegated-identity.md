...
Group resolution follows the same registry-side mechanisms as the device-code flow: SCIM 2.0 push or the `IdpGroupMapping` adapter. SCIM is available on a standard backend, and a standalone server resolves groups through `IdpGroupMapping` alone. The registry reads group membership from the token's `groups` claim, and a deployment whose IdP emits membership under another claim name sets `identity_provider.groups_claim` (`PODIUM_OAUTH_GROUPS_CLAIM`) to that name. The `IdpGroupMapping` adapter maps the values the claim carries and names no claim.

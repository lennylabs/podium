...
Groups come from `X-Podium-User-Groups` directly, and `IdpGroupMapping` is not consulted, because there is no token to read and the gateway is the source of truth. Provision groups at the gateway for a `trusted-headers` deployment. A directory pushed to the SCIM receiver resolves no layer visibility under this provider: a `groups:` filter matches the `X-Podium-User-Groups` value alone, and a SCIM membership naming the caller's `X-Podium-User-Sub` or `X-Podium-User-Email` value grants nothing. Setting `PODIUM_SCIM_TOKENS` still mounts the receiver, which keeps accepting and serving pushes, and the registry reports the withheld expansion at startup with the line `SCIM group expansion in layer visibility: false (identity provider "trusted-headers")`. A deployment that wants directory membership to grant visibility provisions that membership at the gateway, or runs `oidc-jwt`, where the registry resolves groups from a credential it verifies.

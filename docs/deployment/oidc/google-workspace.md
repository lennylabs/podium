...
For Option C, set `PODIUM_IDP_GROUP_MAPPING` on the registry to a comma-separated list of `<token-value>=<group-name>` pairs, for example `PODIUM_IDP_GROUP_MAPPING=engineering@acme.com=engineering,platform@acme.com=platform`. A claim value with no table entry passes through unchanged, the registry reads the `groups` claim unless `PODIUM_OAUTH_GROUPS_CLAIM` or `identity_provider.groups_claim` names another claim, and the mapping has no `registry.yaml` key. A non-empty value that carries a malformed entry, or that resolves to no `<token-value>=<group-name>` entry at all, fails startup with `config.invalid_idp_group_mapping`. One malformed entry discards the well-formed entries beside it, so the registry refuses the whole setting instead of starting with a partial table. To restrict access to the Workspace domain, place the registry behind an upstream identity-aware proxy that authenticates the human caller and admits only accounts in the domain. Restart the registry.

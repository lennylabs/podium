...
`extends:` is read as a literal top-level key. A reference the child resolves through a YAML merge key (`<<: *base`), or one another key reaches through an alias into an anchored mapping, is refused on the read path. The registry reassembles the merged block and expands every alias in the values it restores, so such a reference lands in the served block as the parent's canonical ID and fails the check above. An anchor on the `extends:` value alone is inert, because the reassembled block is written from the merged fields and carries neither the reference nor the anchor.

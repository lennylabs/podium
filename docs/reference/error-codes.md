...
| `config.invalid` | A `publish.yaml` marketplace output is malformed: its harness set names a non-publish-target harness (`opencode` or `none` have no git-repo distribution), a plugin glob is malformed, or a workflow command declares neither `run:` nor `sh:` (or both). `podium publish` rejects it at config validation. |

...
| `auth.forbidden` | An admin-only operation attempted by a non-admin caller, including a receiver CRUD call (`/v1/webhooks`) by a caller without the per-tenant admin role. Also a layer write the [layer write authorization rule](http-api#layer-management) authorizes on neither arm: an admin-defined layer written by a caller without the per-tenant admin role, or a user-defined layer written by a caller who is neither its stored owner nor a tenant admin, including a caller who resolves no subject, on a registry that configures an identity provider; a registry started with no identity provider configured, or one started in public mode, authenticates no caller and admits the write. Also a registration, a filesystem-path patch, a restore, or a reingest of a layer that names a filesystem path on the registry host, attempted by a caller without the per-tenant admin role, which the [local-source authorization rule](http-api#layer-management) governs; that arm carries `details.constraint: "local_source"` and names no filesystem path; a registry started with no identity provider configured, or one started in public mode, authenticates no caller, so no caller can hold the admin role and these operations are admitted there. A request lands on it in three ways: a `local` `source_type`, a `local_path` carried outside a `git` source, or a `repo` string that resolves to the Git file transport. It differs from `ingest.source_unreachable`, which reports a source the registry tried to read and could not; this code reports a read the registry refused to attempt for that caller. Also a registration that asserts `owner`, `public`, `organization`, `groups`, or `users`, which are read on a tenant admin's registration alone, attempted by a caller without the per-tenant admin role; that arm carries `details.constraint: "admin_only_fields"` and names the asserted fields in its message. A field is asserted by its value rather than by its presence, so `public` or `organization` carrying false, an empty `groups` or `users`, an empty `owner`, and an `owner` naming the caller's own verified subject assert nothing. The registration class field is resolved rather than refused, so a registration asserting none of those fields is stored as a user-defined layer owned by the caller. A registry started with no identity provider configured, or one started in public mode, authenticates no caller and admits every caller on the admin arm, so the rule refuses nothing there. This arm is evaluated after the other two, so a registration the layer write authorization rule refuses carries a bare `auth.forbidden` with no `details.constraint`, a registration also on the local-source arm carries `details.constraint: "local_source"`, and `admin_only_fields` is returned only where neither earlier rule refuses. |
...
| `config.oidc_jwt_audience_unset` | `PODIUM_IDENTITY_PROVIDER=oidc-jwt` with `PODIUM_OAUTH_AUDIENCE` resolving to no audience, because it is unset, empty, or blank after each entry is trimmed. The required `aud` claim cannot be verified. |
| `config.injected_token_audience_unset` | `PODIUM_IDENTITY_PROVIDER=injected-session-token` with `PODIUM_OAUTH_AUDIENCE` resolving to no audience, because it is unset, empty, or blank after each entry is trimmed. The variable must name at least one audience this registry answers to, so the required `aud` claim is verified on every token. |

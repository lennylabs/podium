...
| `--web-ui-auth` | Sign the browser in through the registry with the OAuth authorization-code flow. Requires `--web-ui`, `PODIUM_IDENTITY_PROVIDER=oidc-jwt`, public mode off, and the `PODIUM_WEB_UI_OAUTH_*` acquisition values below; a configuration that fails one of those conjuncts aborts startup with `config.web_ui_auth_unconfigured`. Overrides `PODIUM_WEB_UI_AUTH`. |

...
Each result's `frontmatter` is the artifact's stored YAML frontmatter as a string. For an artifact that declares `extends:`, the `extends` key is removed before the block is returned, so the result does not surface the parent. That block is re-encoded from the remaining keys, which normalizes comments, quoting, and indentation. The block is empty when the stored frontmatter cannot be decoded.

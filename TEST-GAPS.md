...
- **Severity**: P1. **Lane**: opt-in (`PODIUM_STACK_PARITY=1`) plus live Postgres + S3. **Status**: partial (flagged). `test/e2e/standard_stack_parity_test.go` boots standard mode (Postgres + S3 + injected-session-token), and the boot, admin authorization, and git-source `layer register`/`reingest` are validated against a live stack (the layer records `last_ingested_ref` at the cloned commit). The author flow was reworked from `--local` (which a remote server cannot read) to a git source. The consumer journey still does not pass: the reingest parses zero manifests, so the artifact never reaches the metadata store, the vector index, or search (verified empty in Postgres: `manifests` and `vec_artifacts` both 0 for the tenant). This is a standard-mode ingest-pipeline question (why a registered git layer ingests no artifacts) rather than a flake or a source-type issue, and no existing test proves a runtime git layer's artifacts become searchable. The test is gated behind `PODIUM_STACK_PARITY=1` so it does not break CI, flagged for follow-up on the ingest pipeline.

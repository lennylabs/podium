...
// entry. The allowlist overrides the private-address rejection so a
// deployment can reach a legitimately internal receiver; it does not
// waive the https requirement, which applies to every target. A rejection
// is a *DisallowedTargetError wrapping ErrDisallowedTarget. Registration
// and delivery both call Validate so the two agree.
func (p *URLPolicy) Validate(ctx context.Context, rawURL string) error {
	...
	// A host on the allowlist is permitted without resolving it. The
	// allowlist overrides only the resolved-address rejection; the https
	// requirement above applies regardless.

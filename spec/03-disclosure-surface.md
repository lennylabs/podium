...
The host calls `load_domain(path)` to get a map of what exists. With no path, the map describes top-level domains. With a path like `finance`, it describes that domain's subdomains and notable artifacts. The rendering of the map (depth, folding of sparse subdomains, ordering of notable entries, response-size budget) is governed by the discovery rules in §4.5.5, configured at tenant scope in `registry.yaml` and overridable per-subtree via `DOMAIN.md`. The directory layout defines the domain hierarchy (§4.2); a domain's children may be augmented or curated by an optional `DOMAIN.md` config that imports artifacts from elsewhere (§4.5). Multi-membership is allowed: one artifact can show up under more than one domain via imports.

When the host doesn't know which domain to start in, it calls `search_domains(query)` instead. Hybrid retrieval (BM25 + embeddings, fused via reciprocal rank) runs over each domain's projection (frontmatter `description` + `keywords` + truncated prose body, per §4.7 *Embedding generation*) and returns ranked domain descriptors. The host picks one and calls `load_domain` on its path to drill in.
...
The caller's OAuth identity determines layer composition exactly as for a real session; the preview is a read-only projection of that composition with counts only.

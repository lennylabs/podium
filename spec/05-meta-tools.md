...
The MCP server declares its capabilities in the MCP `initialize` response: `{tools: true, resources: true, sessionCorrelation: true}`. The `resources` capability backs the read-only artifact-body mirror described in §5.0.

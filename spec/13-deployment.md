...
| `PODIUM_OAUTH_SUBJECT_CLAIM` | Claim read as the caller's subject in place of `sub`. The named claim takes precedence and `sub` is the fallback. An AD FS deployment sets `idsub`, because AD FS access tokens carry no `sub`. Config-file key `identity_provider.subject_claim`. Read only under `oidc-jwt`. | (unset; `sub`) |
| `PODIUM_OAUTH_GROUPS_CLAIM` | Claim read for group membership in place of `groups`. An AD FS deployment sets the full claim-type URI its issuance rules emit. The claim is read in the multi-value form and in the single-string form an IdP emits for a caller in exactly one group. Config-file key `identity_provider.groups_claim`. Read only under `oidc-jwt`. | (unset; `groups`) |

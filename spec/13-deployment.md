...
None of these keys carries a config-file form. The registry's resolved audience, which the sign-in redirect sends, is configured through the `oidc-jwt` keys `PODIUM_OAUTH_AUDIENCE` or `identity_provider.audience` (§6.3.3, §13.12) rather than through a web-UI key.
...
The registry-process providers (§6.3.3) and the `injected-session-token` provider (§6.3.2) introduce the following registry-process variables. `oidc-jwt` also reuses `PODIUM_OAUTH_AUDIENCE` (§6.3) for the `aud` claim, which it requires.

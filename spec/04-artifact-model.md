...
- **`git`**: a remote Git repository at a tracked ref, optionally rooted at a subpath. The registry ingests on webhook (§7.3.1). Provider-level details (signature verification, fetch semantics) flow through the `GitProvider` SPI (§9.1), with built-in support for GitHub, GitLab, Bitbucket.

...
**Deprecated candidates in an `extends:` reference.** A range or unpinned `extends:` reference selects among non-deprecated versions only. Deprecated versions are removed from the candidate set before resolution, and `latest` for such a reference is the most recently ingested non-deprecated version. When the parent has at least one stored version and the filter leaves no candidate, ingest rejects the child with `ingest.invalid_artifact` (§6.10) and reports that the parent's candidate versions are deprecated; the reference does not fall back to the deprecated versions. The refusal applies at ingest and leaves the resolution of an already-stored artifact unchanged.

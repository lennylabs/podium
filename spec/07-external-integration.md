...
`GET /v1/ui/session` reports the deployment's identity posture, the caller's own resolved subject, and what that caller may do on the §7.3.1 layer operations. It requires no credential and refuses no request for lack of one; a request that carries one has it verified so the response can report `subject` and evaluate `layer_capabilities`, and for no other purpose, and a request that resolves no subject is answered `200` with `subject` absent. The body carries:

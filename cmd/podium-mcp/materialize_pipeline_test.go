...
// ----- F-6.6.2 content-hash verification ----------------------------------
...
// Spec: §6.6 step 2 — skills are skipped (their content_hash covers the
// SKILL.md bytes the registry discards at ingest), so a skill is not
// false-rejected by the local check.
func TestDeliver_SkillSkipsContentHashCheck(t *testing.T) {
	t.Parallel()
	dest := t.TempDir()
	s := newTestServer(t, &config{harness: "none", materializeRoot: dest, verifyPolicy: sign.PolicyNever})
	resp := loadArtifactResponse{
		...
		ContentHash:  "sha256:" + strings.Repeat("0", 64), // would mismatch if checked
	}
	out := s.deliverLoadArtifact(resp, deliverOpts{harness: "none", destination: dest})
	m := out.(map[string]any)
	if code, _ := m["code"].(string); code == "materialize.content_hash_mismatch" {
		t.Errorf("skill was content-hash-checked and rejected: %v", m)
	}
}

// Spec: §6.6 step 2 / §4.6 — an extends-merged manifest (manifest_merged) is
// skipped because its served frontmatter is a re-serialization that cannot
// reproduce the stored hash.
func TestDeliver_MergedManifestSkipsContentHashCheck(t *testing.T) {
	t.Parallel()
	dest := t.TempDir()
	s := newTestServer(t, &config{harness: "none", materializeRoot: dest, verifyPolicy: sign.PolicyNever})
	resp := fixtureResp("team/x", "---\ntype: context\n---\nbody")
	resp.ContentHash = "sha256:" + strings.Repeat("0", 64) // would mismatch
	resp.ManifestMerged = true
	out := s.deliverLoadArtifact(resp, deliverOpts{harness: "none", destination: dest})
	m := out.(map[string]any)
	if code, _ := m["code"].(string); code == "materialize.content_hash_mismatch" {
		t.Errorf("merged manifest was content-hash-checked and rejected: %v", m)
	}
}
